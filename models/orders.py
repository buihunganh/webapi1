...
ORDER_SELECT = '*, customer:customerid(userid,fullname,email,phone), shipper:shipperid(userid,fullname,email,phone), address:addressid(addressid,fulladdress,city)'
...
def get_orders_for_user(*, user_id: int, role: str, limit: int = 200):
    """SELECT đơn hàng theo quyền truy cập role tại server."""
    sb = get_supabase()
    query = sb.table('orders').select(ORDER_SELECT).order('orderdate', desc=True).limit(limit)

    if role == 'customer':
        query = query.eq('customerid', user_id)
    elif role == 'shipper':
        # Existing shipper policy: see assigned orders and pending queue.
        query = query.or_(f'shipperid.eq.{user_id},orderstatus.eq.pending')
...
def get_order_by_id(order_id: int):
    """Lấy 1 order kèm customer + address + shipper thông tin cơ bản."""
    sb = get_supabase()
    response = (
        sb.table('orders')
        .select(ORDER_SELECT)
...
def get_order_items(order_id: int):
    """Lấy item list cho màn chi tiết đơn hàng."""
    sb = get_supabase()
    response = (
        sb.table('orderdetails')
        .select('orderdetailid, orderid, productid, quantity, unitprice, product:productid(productid,productname,imageurl)')
        .eq('orderid', order_id)
        .order('orderdetailid')
        .execute()
    )
...
def get_order_payment(order_id: int):
    ...
    response = (
        sb.table('payments')
        .select('paymentid, amount, method, status, paidat')
        .eq('orderid', order_id)
