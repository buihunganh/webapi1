...
function resolveFallbackEmoji(product) {
...
async function resolveDisplayImageUrl(imageUrl) {
  ...
  if (window.SupabaseWeb && typeof window.SupabaseWeb.getPublicStorageUrl === 'function') {
    try {
      return await window.SupabaseWeb.getPublicStorageUrl(raw, 'product-images');
    } catch (err) {
      console.warn('Cannot convert storage path to public URL:', err);
    }
  }
...
async function loadProductsFromAPI() {
  try {
    ...
    if (apiProducts && apiProducts.length > 0) {
      ...
      PRODUCTS = await Promise.all(apiProducts.map(async (p, idx) => ({
        ...
        rating: 4.5,
        reviews: [],
        imageurl: await resolveDisplayImageUrl(p.imageurl || ''),
      })));
