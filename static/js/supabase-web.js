(function () {
  ...
  async function getPublicStorageUrl(path, bucketName = "product-images") {
    const rawPath = String(path || "").trim();
    if (!rawPath) return "";
    if (/^(https?:)?\/\//i.test(rawPath) || /^data:image\//i.test(rawPath) || /^blob:/i.test(rawPath)) {
      return rawPath;
    }

    const cfg = await fetchConfig();
    const normalizedPath = rawPath.replace(/^\/+/, "");
    return `${cfg.supabase_url}/storage/v1/object/public/${bucketName}/${normalizedPath}`;
  }
