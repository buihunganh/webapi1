(function () {
  ...
  async function fetchProductsViaRest(limit = 100) {
    const cfg = await fetchConfig();
    const url = `${cfg.supabase_url}/rest/v1/products?select=productid,productname,description,price,categoryid,isactive,imageurl&order=productid.asc&limit=${encodeURIComponent(limit)}`;
  ...
  async function uploadProductImage(file, productId, bucketName = 'product-images') {
